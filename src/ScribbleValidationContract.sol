...
contract ScribbleValidationContract {
    ...
    function deposit(uint256 amount) external {
        ...
        unchecked {
            ...
            if (!(totalDeposits == (_v.old_1 + amount))) {
                emit __ScribbleUtilsLib__127.AssertionFailed(
                    "001496:0105:000 4: Total deposits must increase by amount"
                );
                assert(false);
            }
    ...
    function __scribble_ScribbleValidationContract_check_state_invariants_internal() internal {
        unchecked {
            if (!(balance <= totalDeposits)) {
                emit __ScribbleUtilsLib__127.AssertionFailed(
                    "004027:0116:000 0: Balance must equal or be less than total deposits"
                );
                assert(false);
            }
            if (!(totalDeposits >= 0)) {
    ...
    function __scribble_check_state_invariants() internal virtual {
