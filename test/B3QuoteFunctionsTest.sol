...
contract B3QuoteFunctionsTest is Test {
    ...
    function testQuoteAddLiquidityBasic() public view {
        ...
        uint quotedAmount = b3.quoteAddLiquidity(inputAmount);
    ...
    function testQuoteAddLiquidityMath() public view {
        ...
        (uint currentVirtualInput, uint currentVirtualL,) = b3.getVirtualPair();
        ...
        uint denominator = currentVirtualInput + inputAmount + alpha;
        uint expectedNewVirtualL = virtualK / denominator - beta;
        uint expectedQuote = currentVirtualL - expectedNewVirtualL;

        uint actualQuote = b3.quoteAddLiquidity(inputAmount);

        // Use precision tolerance for large numbers
        uint tolerance = 1e14; // 0.01% tolerance for 1e18 scale
    ...
    function testQuoteAddLiquidityZeroInput() public view {
        uint quote = b3.quoteAddLiquidity(0);
    ...
    function testQuoteAddLiquiditySmallAmount() public view {
        uint inputAmount = 1e18; // 1 token

        uint quote = b3.quoteAddLiquidity(inputAmount);

        assertTrue(quote > 0, "Should handle small amounts");
    }

    function testQuoteAddLiquidityLargeAmount() public view {
        uint inputAmount = 1000; // Large amount relative to virtual pair scale

        uint quote = b3.quoteAddLiquidity(inputAmount);
        (, uint currentVirtualL,) = b3.getVirtualPair();
    ...
    function testQuoteAddLiquidityDifferentAmounts() public view {
        ...
        for (uint i = 0; i < amounts.length; i++) {
            uint quote = b3.quoteAddLiquidity(amounts[i]);
    ...
    function testQuoteRemoveLiquidityBasic() public view {
        ...
        uint quotedAmount = b3.quoteRemoveLiquidity(bondingAmount);
    ...
    function testQuoteRemoveLiquidityMath() public view {
        ...
        uint denominator = currentVirtualL + bondingAmount + beta;
        uint expectedNewVirtualInput = virtualK / denominator - alpha;
        uint expectedQuote = currentVirtualInput - expectedNewVirtualInput;

        uint actualQuote = b3.quoteRemoveLiquidity(bondingAmount);

        // Use precision tolerance for large numbers
        uint tolerance = 1000; // Smaller tolerance for smaller amounts
        assertApproxEqAbs(actualQuote, expectedQuote, tolerance, "Quote should match calculated value within tolerance");
    }

    function testQuoteRemoveLiquidityZeroInput() public view {
        uint quote = b3.quoteRemoveLiquidity(0);
    ...
    function testQuoteRemoveLiquiditySmallAmount() public view {
        uint bondingAmount = 100; // Small amount

        uint quote = b3.quoteRemoveLiquidity(bondingAmount);

        assertTrue(quote > 0, "Should handle small amounts");
    }

    function testQuoteRemoveLiquidityLargeAmount() public view {
        uint bondingAmount = 50_000_000; // Large amount

        uint quote = b3.quoteRemoveLiquidity(bondingAmount);
        (uint currentVirtualInput,,) = b3.getVirtualPair();
    ...
    function testQuoteRemoveLiquidityDifferentAmounts() public view {
        ...
        for (uint i = 0; i < amounts.length; i++) {
            uint quote = b3.quoteRemoveLiquidity(amounts[i]);
    ...
    function testQuoteAddLiquidityConsistentWithActual() public {
        ...
        uint actualAmount = b3.addLiquidity(inputAmount, 0);
        vm.stopPrank();
    ...
    function testQuoteRemoveLiquidityConsistentWithActual() public {
        ...
        uint bondingTokens = b3.addLiquidity(1000 * 1e18, 0);
        vm.stopPrank();

        // Get quote for removal
        uint quotedAmount = b3.quoteRemoveLiquidity(bondingTokens);

        // Perform actual removal
        vm.startPrank(user1);
        uint actualAmount = b3.removeLiquidity(bondingTokens, 0);
        vm.stopPrank();
    ...
    function testQuotesDoNotChangeState() public {
        // Store initial state
        (uint initialVInput, uint initialVL, uint initialK) = b3.getVirtualPair();
        uint initialBondingSupply = bondingToken.totalSupply();
        ...
        (uint finalVInput, uint finalVL, uint finalK) = b3.getVirtualPair();
        uint finalBondingSupply = bondingToken.totalSupply();
    ...
    function testQuoteAddLiquidityMaxInput() public view {
        ...
        uint maxInput = type(uint).max / 2; // Avoid overflow

        try b3.quoteAddLiquidity(maxInput) returns (uint quote) {
    ...
    function testQuoteRemoveLiquidityMaxBonding() public view {
        ...
        uint maxBonding = type(uint).max / 2; // Avoid overflow

        try b3.quoteRemoveLiquidity(maxBonding) returns (uint quote) {
    ...
    function testQuoteAddLiquidityPrecision() public view {
        uint smallAmount = 1; // Smallest possible amount

        uint quote = b3.quoteAddLiquidity(smallAmount);
    ...
    function testQuoteRemoveLiquidityPrecision() public view {
        ...
        uint quote = b3.quoteRemoveLiquidity(smallAmount);
    ...
    function testMultipleQuoteCallsConsistent() public view {
        ...
        uint quote1 = b3.quoteAddLiquidity(inputAmount);
        uint quote2 = b3.quoteAddLiquidity(inputAmount);
        uint quote3 = b3.quoteAddLiquidity(inputAmount);
    ...
    function testQuoteAfterStateChanges() public {
        ...
        // Get initial quote
        uint initialQuote = b3.quoteAddLiquidity(inputAmount);
        ...
        uint newQuote = b3.quoteAddLiquidity(inputAmount);
    ...
    function testQuoteSymmetryAddRemove() public {
        ...
        // Quote add liquidity
        uint bondingQuote = b3.quoteAddLiquidity(inputAmount);

        // Quote remove liquidity with the quoted amount
        uint inputQuote = b3.quoteRemoveLiquidity(bondingQuote);
